...
import { pharmacyProfiles } from '../../data/adminData'
...
export default function PaymentConfigPage({ user }) {
  ...
  const [banco, setBanco] = useState(profile?.pagoMovil?.banco || '')
  const [telefono, setTelefono] = useState(profile?.pagoMovil?.telefono || '')
  const [ci, setCi] = useState(profile?.pagoMovil?.ci || '')
  const [titular, setTitular] = useState(profile?.pagoMovil?.titular || profile?.nombre || '')
  const [saved, setSaved] = useState(false)
  const [copyMsg, setCopyMsg] = useState('')

  function handleSave() {
    const data = { banco, telefono, ci, titular }
    localStorage.setItem('pharmacy_pagoMovil_' + farmaciaId, JSON.stringify(data))
    setSaved(true)
    ...
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <div className="bg-white rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.06)] border border-gray-100 p-6 space-y-5">
        <div className="flex items-center gap-3 pb-4 border-b border-gray-100">
          ...
          <div>
            <div className="text-sm font-bold text-gray-800">{profile?.nombre || 'Mi Farmacia'}</div>
        ...
        <div>
          ...
          <input
            ...
            className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 text-sm"
          />
        </div>
        ...
        <button
          onClick={handleSave}
          className="w-full bg-gradient-to-r from-sky-600 to-blue-700 hover:from-sky-700 hover:to-blue-800 text-white font-bold py-3 rounded-xl transition-all duration-200 shadow-lg shadow-blue-500/25 active:scale-[0.97] text-sm"
        >
