...
export default function ShoppingPage({ user, onLogout }) {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gradient-to-br from-slate-50 via-white to-sky-50">
      <header className="flex-shrink-0 flex items-center justify-between px-4 sm:px-6 py-2 sm:py-3 border-b border-gray-100 bg-white backdrop-blur-xl">
        <div className="flex items-center gap-2 sm:gap-4">
          <div className="relative">
            <div className="absolute inset-0 bg-gradient-to-br from-sky-400 to-blue-600 rounded-2xl blur-lg opacity-40" />
            <div className="relative bg-gradient-to-br from-sky-500 to-blue-600 rounded-xl sm:rounded-2xl p-2 sm:p-3 border border-white/20">
              <img src={logo} alt="Dron Salud" className="w-8 h-8 sm:w-12 sm:h-12 object-contain" />
            </div>
          </div>
          <div>
            <div className="text-[8px] sm:text-[10px] uppercase tracking-[0.2em] sm:tracking-[0.3em] text-blue-500 font-semibold">Log&iacute;stica M&eacute;dica</div>
            <div className="text-sm sm:text-lg font-bold text-gray-800 font-['Plus_Jakarta_Sans']">Inteligente</div>
          </div>
        </div>
      ...
      <nav className="flex-shrink-0 flex items-center gap-1 px-4 sm:px-6 py-2 sm:py-3 border-b border-gray-100 bg-white/30 backdrop-blur-sm overflow-x-auto">
        {tabs.map((tab) => (
          <button
            key={tab.key}
            onClick={() => setActiveTab(tab.key)}
            className={`flex items-center gap-2 px-4 sm:px-5 py-2 sm:py-2.5 rounded-xl text-xs sm:text-sm font-semibold transition-all duration-200 whitespace-nowrap ${
              activeTab === tab.key
                ? 'bg-gradient-to-r from-sky-600 to-blue-700 text-white shadow-md'
                : 'text-gray-600 hover:text-blue-600 hover:bg-blue-50'
            }`}
          >
            <span className="text-base sm:text-lg">{tab.key === 'shop' ? '\uD83D\uDED2' : '\uD83D\uDCCB'}</span>
            <span>{tab.label}</span>
          </button>
        ))}
      </nav>
      ...
      {showProfile && (
        <div className="fixed sm:absolute top-16 right-2 sm:right-6 z-50 animate-scale-in">
          <div className="bg-white rounded-2xl shadow-xl border border-gray-100 p-5 min-w-[200px] max-w-[calc(100vw-16px)]">
