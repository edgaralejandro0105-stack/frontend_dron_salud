...
function ConfirmModal({ message, onConfirm, onCancel, confirmText = 'Confirmar', confirmColor = 'from-rose-500 to-rose-600' }) {
  return (
    <div className="fixed inset-0 z-[80] flex items-center justify-center bg-black/40 backdrop-blur-sm" onClick={onCancel}>
      <div className="bg-white dark:bg-slate-900 rounded-3xl shadow-2xl p-8 max-w-sm mx-4 w-full animate-scale-in" onClick={(e) => e.stopPropagation()}>
        <p className="text-gray-800 dark:text-slate-200 text-sm font-semibold text-center mb-6">{message}</p>
        <div className="flex gap-3">
          <button onClick={onCancel} className="flex-1 bg-gray-100 dark:bg-slate-800 hover:bg-gray-200 dark:hover:bg-slate-700 text-gray-700 dark:text-slate-300 font-bold py-3 rounded-xl transition-all">Volver</button>
...
export default function DispatchPage({ user }) {
  ...
  return (
    <div className="space-y-6">
      <div className="card-hover bg-white dark:bg-slate-800/90 rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.06)] dark:shadow-[0_8px_30px_rgb(0,0,0,0.25)] border border-gray-100 dark:border-slate-700/50 p-5">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-sm font-bold text-gray-800 dark:text-slate-200 font-['Plus_Jakarta_Sans']">Pedidos Pendientes</h3>
        ...
        {pendingOrders.length === 0 ? (
          <div className="text-center py-6 text-gray-400 dark:text-slate-500 text-sm">No hay pedidos pendientes</div>
        ) : (
            ...
            {pendingOrders.map(o => {
              ...
              return (
                <div key={o.id_pedido} className={`p-4 rounded-2xl border transition-all duration-200 ${
                  isAccepted ? 'border-blue-300 dark:border-blue-600 bg-gradient-to-br from-sky-50/80 to-blue-50/80 dark:from-sky-900/20 dark:to-blue-900/20 shadow-md' : 'border-gray-100 dark:border-slate-700/50 bg-white/80 dark:bg-slate-800/50 hover:bg-blue-50/30 dark:hover:bg-slate-700/50 hover:border-blue-200 dark:hover:border-blue-600/50 shadow-sm'
                }`}>
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-sm font-bold text-gray-800 dark:text-slate-200">#{o.id_pedido}</span>
                    <Badge text={o.estado_pedido} />
                  </div>
                  <div className="text-xs text-gray-600 dark:text-slate-400 mb-1">{p?.nombre_comercial || `Farmacia #${o.id_farmacia}`}</div>
                  <div className="text-xs font-semibold text-gray-700 dark:text-slate-300">{o.cliente?.nombre} {o.cliente?.apellido || ''}</div>
                  <div className="text-xs text-gray-500 dark:text-slate-400">{(o.detalles || []).length} producto{(o.detalles || []).length !== 1 ? 's' : ''} · {formatCurrency(o.total)}</div>
                  ...
                  <button
                    ...
                    className={`mt-3 w-full py-2 rounded-xl text-xs font-bold transition-all duration-200 active:scale-[0.97] ${
                      o.estado_pedido === 'Preparado'
                        ...
                        : 'bg-gray-100 dark:bg-slate-700 text-gray-500 dark:text-slate-400 border border-gray-200 dark:border-slate-600 hover:bg-gray-200 dark:hover:bg-slate-600'
                    }`}
                  >
      ...
      {order && profile ? (
        <div className="grid gap-6 grid-cols-1 lg:grid-cols-[1fr_1.1fr]">
          <div className="space-y-5">
            <div className="card-hover bg-white dark:bg-slate-800/90 rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.06)] dark:shadow-[0_8px_30px_rgb(0,0,0,0.25)] border border-gray-100 dark:border-slate-700/50 p-5">
              <div className="flex items-center justify-between mb-3">
                <h3 className="text-sm font-bold text-gray-800 dark:text-slate-200 font-['Plus_Jakarta_Sans']">Ruta de Entrega</h3>
              ...
              <div className="flex items-center gap-2 mb-3 bg-gray-50 dark:bg-slate-900/50 rounded-xl px-4 py-2.5 border border-gray-100 dark:border-slate-700 text-xs">
                <span className="w-5 h-5 rounded-full bg-green-100 dark:bg-green-900/30 flex items-center justify-center text-green-700 dark:text-green-400 font-bold text-[10px]">O</span>
                <span className="text-gray-600 dark:text-slate-400 truncate">{profile.nombre_comercial}</span>
                <span className="text-gray-300 dark:text-slate-600">-</span>
                <span className="w-5 h-5 rounded-full bg-blue-100 dark:bg-blue-900/30 flex items-center justify-center text-blue-700 dark:text-blue-400 font-bold text-[10px]">D</span>
                <span className="text-gray-600 dark:text-slate-400 truncate">{order.destino_nombre || 'Destino'}</span>
              </div>
              <div className="rounded-2xl overflow-hidden border border-gray-200 dark:border-slate-700 h-[200px] sm:h-[290px]">
            ...
            <div className="card-hover bg-white dark:bg-slate-800/90 rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.06)] dark:shadow-[0_8px_30px_rgb(0,0,0,0.25)] border border-gray-100 dark:border-slate-700/50 p-5">
              <div className="flex items-center justify-between mb-3">
                <h3 className="text-sm font-bold text-gray-800 dark:text-slate-200 font-['Plus_Jakarta_Sans']">Ubicacion de la Farmacia</h3>
              ...
              <div className="flex items-center gap-3 mb-3 bg-green-50 dark:bg-green-900/20 rounded-xl px-4 py-2.5 border border-green-100 dark:border-green-800/30 text-xs">
                <div className="w-7 h-7 rounded-full bg-green-200 dark:bg-green-800/40 flex items-center justify-center text-green-800 dark:text-green-300 font-bold text-sm">{profile.nombre_comercial?.charAt(0)}</div>
                <div>
                  <div className="font-bold text-gray-800 dark:text-slate-200">{profile.nombre_comercial}</div>
                  <div className="text-gray-500 dark:text-slate-400">{profile.direccion}</div>
                </div>
              </div>
              <div className="rounded-2xl overflow-hidden border border-gray-200 dark:border-slate-700 h-[200px] sm:h-[290px]">
          ...
          <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <div>
                      <div className="text-xs font-bold text-gray-800 dark:text-slate-200">Origen</div>
                ...
                <div>
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <div>
                      <div className="text-xs font-bold text-gray-800 dark:text-slate-200">Destino</div>
            ...
            <div className="card-hover bg-white dark:bg-slate-800/90 rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.06)] dark:shadow-[0_8px_30px_rgb(0,0,0,0.25)] border border-gray-100 dark:border-slate-700/50 p-5">
              <div className="flex items-center justify-between mb-3">
                <h3 className="text-sm font-bold text-gray-800 dark:text-slate-200 font-['Plus_Jakarta_Sans']">#{order.id_pedido}</h3>
              ...
              <div className="space-y-1.5 mb-3">
                {(order.detalles || []).map((p, i) => (
                  <div key={i} className="flex items-center justify-between text-xs text-gray-600">
                    ...
                    <span className="font-semibold text-gray-800 dark:text-slate-200">{formatCurrency(Number(p.precio_unitario) * p.cantidad)}</span>
                  </div>
                ))}
              </div>
              ...
              <div className="flex items-center justify-between text-sm border-t border-gray-100 pt-3 mt-1">
                ...
                <span className="font-bold text-gray-800 dark:text-slate-200">{formatCurrency(order.total)}</span>
              </div>
            </div>
            ...
            {order.cliente && (
              <div className="card-hover bg-white dark:bg-slate-800/90 rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.06)] dark:shadow-[0_8px_30px_rgb(0,0,0,0.25)] border border-gray-100 dark:border-slate-700/50 p-5">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <h3 className="text-sm font-bold text-gray-800 dark:text-slate-200 font-['Plus_Jakarta_Sans']">Datos del Cliente</h3>
                </div>
                <div className="text-xs text-gray-600 space-y-2">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-semibold text-gray-800 dark:text-slate-200">{order.cliente.nombre} {order.cliente.apellido || ''}</span>
                  </div>
                  <div className="flex items-center gap-2">
            ...
            <div className="card-hover bg-white dark:bg-slate-800/90 rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.06)] dark:shadow-[0_8px_30px_rgb(0,0,0,0.25)] border border-gray-100 dark:border-slate-700/50 p-5">
              <h3 className="text-sm font-bold text-gray-800 dark:text-slate-200 font-['Plus_Jakarta_Sans'] mb-3">Seleccionar Dron</h3>
              {enVuelo && launchedDrone ? (
                <div className="flex items-center gap-3 p-3 rounded-xl bg-gradient-to-br from-sky-50 to-blue-50 dark:from-sky-900/20 dark:to-blue-900/20 border border-sky-200 dark:border-sky-800/30 mb-3">
                  ...
                  <div>
                    <div className="text-sm font-bold text-sky-800 dark:text-sky-300">Dron en vuelo</div>
                    <div className="text-xs text-sky-600 dark:text-sky-400">Disponible al finalizar el pedido</div>
                  </div>
                </div>
              ) : (
                <>
                  <div className="space-y-2 mb-3">
                    {drones.length === 0 ? (
                      <p className="text-xs text-gray-400 dark:text-slate-500 text-center py-4">No hay drones disponibles</p>
                    ) : (
                      drones.map(d => (
                        <label key={d.id_dron} className={`flex items-center gap-3 p-2.5 rounded-xl border cursor-pointer transition-all ${selectedDrone === String(d.id_dron) ? 'border-blue-300 dark:border-blue-600 bg-gradient-to-br from-sky-50/80 to-blue-50/80 dark:from-sky-900/30 dark:to-blue-900/30 shadow-sm' : 'border-gray-100 dark:border-slate-700 bg-white/80 dark:bg-slate-800/50 hover:bg-blue-50/30 dark:hover:bg-slate-700/50 hover:border-blue-200 dark:hover:border-blue-600/50'}`}>
                          ...
                          <div className="flex-1">
                            <div className="text-sm font-bold text-gray-800 dark:text-slate-200">{d.matricula}</div>
                             <div className="text-xs text-gray-500 dark:text-slate-400">{d.modelo}</div>
                          </div>
                        </label>
                      ))
                    )}
                  </div>
                   <button onClick={handleLaunch} disabled={!selectedDrone} className={`w-full py-3 rounded-xl text-sm font-bold transition-all active:scale-[0.97] ${selectedDrone ? 'bg-gradient-to-r from-sky-600 to-blue-700 text-white shadow-lg' : 'bg-gray-100 dark:bg-slate-700 text-gray-400 dark:text-slate-500 cursor-not-allowed'}`}>
              ...
              {order.estado_pedido === 'En transito' && (
                <>
                  ...
                  {!enVuelo && (
                   ...
                   <p className="text-[10px] text-gray-400 dark:text-slate-500 text-center mt-1">{enVuelo ? 'El dron está en vuelo' : 'El dron y pedido volverán a estar disponibles'}</p>
                </>
              )}
            </div>
          </div>
        ...
        <div className="bg-white dark:bg-slate-800/90 border border-gray-200 dark:border-slate-700 rounded-3xl shadow-sm p-12 text-center text-gray-400 dark:text-slate-500">
