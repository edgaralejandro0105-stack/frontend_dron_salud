  ...
  return (
    <>
      <div className="card-hover bg-white rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.06)] border border-gray-100">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between p-6 pb-4 border-b border-gray-100 gap-3">
          <div>
            <h3 className="text-lg font-bold text-gray-900 font-['Plus_Jakarta_Sans']">Flota de Drones</h3>
            <p className="text-sm text-gray-500 mt-0.5">{total} drone{total !== 1 ? 's' : ''} registrado{total !== 1 ? 's' : ''}</p>
          </div>
          <div className="flex items-center gap-2">
            <div className="relative">
              <svg className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>
              <input type="text" value={searchInput} onChange={e => setSearchInput(e.target.value)} placeholder="Buscar dron..." className="pl-10 pr-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-200 focus:border-blue-300 transition-all w-48" />
            </div>
        ...
        <div className="p-6">
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-between mt-6 pt-4 border-t border-gray-100">
              <p className="text-xs text-gray-500">Mostrando página {page} de {totalPages} ({total} drones)</p>
              <div className="flex items-center gap-1">
                <button onClick={() => loadDrones(page - 1, search)} disabled={page <= 1} className="px-3 py-2 rounded-lg text-sm font-medium text-gray-600 hover:bg-gray-100 disabled:opacity-30 disabled:cursor-not-allowed transition-all">Anterior</button>
                {Array.from({ length: totalPages }, (_, i) => i + 1).map(p => (
                  <button key={p} onClick={() => loadDrones(p, search)} className={`w-9 h-9 rounded-lg text-sm font-bold transition-all ${p === page ? 'bg-gradient-to-br from-sky-500 to-blue-600 text-white shadow-md shadow-blue-500/20' : 'text-gray-600 hover:bg-gray-100'}`}>{p}</button>
                ))}
                <button onClick={() => loadDrones(page + 1, search)} disabled={page >= totalPages} className="px-3 py-2 rounded-lg text-sm font-medium text-gray-600 hover:bg-gray-100 disabled:opacity-30 disabled:cursor-not-allowed transition-all">Siguiente</button>
              </div>
            </div>
          )}
        </div>
      ...
      {confirmDelete && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
          <div className="bg-white rounded-3xl shadow-2xl border border-gray-100 p-6 w-full max-w-sm text-center">
            ...
            <p className="text-sm text-gray-500 mb-1">Se eliminará el dron "{confirmDelete.matricula}" ({confirmDelete.modelo})</p>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          {mantenimientos.length === 0 && pedidos.length === 0 ? (
            ...
            <>
              ...
              {pedidos.length > 0 && (
                <div>
                  <h4 className="text-xs font-bold text-gray-500 uppercase tracking-widest mb-3 flex items-center gap-2"><svg className="w-4 h-4 text-sky-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" /></svg>Envios Realizados ({pedidos.length})</h4>
                  <div className="space-y-2">{pedidos.map(p => (
                    <div key={p.id_pedido} className="bg-sky-50/50 rounded-xl p-4 border border-sky-100">
                      ...
                      {p.farmacia && <p className="text-xs text-gray-600">{p.farmacia.nombre_comercial}</p>}
                      <div className="flex items-center gap-4 mt-1 text-xs text-gray-500">{p.estado_pedido && <span className={`px-2 py-0.5 rounded-full font-bold ${p.estado_pedido === 'Entregado' ? 'bg-emerald-100 text-emerald-700' : 'bg-gray-100 text-gray-600'}`}>{p.estado_pedido}</span>}{p.total != null && <span>Total: Bs. {Number(p.total).toLocaleString('es-VE', { minimumFractionDigits: 2 })}</span>}</div>
                    </div>))}
                  </div>
                </div>
              )}
      ...
      <div className="bg-white rounded-3xl shadow-xl p-8 w-full max-w-2xl mx-4 max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div className="grid grid-cols-2 gap-4">
            <div><label className="text-xs font-semibold text-gray-600 uppercase tracking-widest mb-1 block">Matricula</label><input name="matricula" value={form.matricula} onChange={handleChange} placeholder="DRN-001" className="w-full px-3 py-2.5 border border-gray-200 rounded-lg text-sm" required /></div>
            <div><label className="text-xs font-semibold text-gray-600 uppercase tracking-widest mb-1 block">Modelo</label><input name="modelo" value={form.modelo} onChange={handleChange} placeholder="DJI M300" className="w-full px-3 py-2.5 border border-gray-200 rounded-lg text-sm" required /></div>
            <div><label className="text-xs font-semibold text-gray-600 uppercase tracking-widest mb-1 block">Fabricante</label><input name="fabricante" value={form.fabricante} onChange={handleChange} placeholder="DJI" className="w-full px-3 py-2.5 border border-gray-200 rounded-lg text-sm" /></div>
            <div><label className="text-xs font-semibold text-gray-600 uppercase tracking-widest mb-1 block">Numero de Serie</label><input name="numero_serie" value={form.numero_serie} onChange={handleChange} placeholder="SN-001" className="w-full px-3 py-2.5 border border-gray-200 rounded-lg text-sm" /></div>
            <div><label className="text-xs font-semibold text-gray-600 uppercase tracking-widest mb-1 block">Peso Max. Despegue (kg)</label><input name="peso_maximo_despegue_kg" type="number" step="0.01" value={form.peso_maximo_despegue_kg} onChange={handleChange} className="w-full px-3 py-2.5 border border-gray-200 rounded-lg text-sm" /></div>
            <div><label className="text-xs font-semibold text-gray-600 uppercase tracking-widest mb-1 block">Horas de Vuelo</label><input name="horas_vuelo" type="number" step="0.01" value={form.horas_vuelo} onChange={handleChange} className="w-full px-3 py-2.5 border border-gray-200 rounded-lg text-sm" /></div>
            <div><label className="text-xs font-semibold text-gray-600 uppercase tracking-widest mb-1 block">Fecha Adquisicion</label><input name="fecha_adquisicion" type="date" value={form.fecha_adquisicion} onChange={handleChange} className="w-full px-3 py-2.5 border border-gray-200 rounded-lg text-sm" /></div>
            <div>
          ...
          {form.estado_operativo === 'Mantenimiento' && (
            <div>
              ...
              <textarea name="motivo_mantenimiento" value={form.motivo_mantenimiento} onChange={handleChange} placeholder="Describa el motivo del mantenimiento..." rows={3} className="w-full px-3 py-2.5 border border-gray-200 rounded-lg text-sm resize-none" required />
            </div>
          )}
          <div>
            <label className="text-xs font-semibold text-gray-600 uppercase tracking-widest mb-1 block">Foto del Dron</label>
            <div className="flex items-center gap-4">
              {form.foto_url && <img src={form.foto_url} alt="Preview" className="w-16 h-16 rounded-xl object-cover border border-gray-200" />}
              <button type="button" onClick={() => fileRef.current?.click()} disabled={uploading} className="px-4 py-2.5 bg-gray-100 hover:bg-gray-200 rounded-xl text-sm font-medium text-gray-700 transition-colors">{uploading ? 'Subiendo...' : 'Subir imagen'}</button>
              <input ref={fileRef} type="file" accept="image/*" className="hidden" onChange={handleUpload} />
            </div>
          </div>
          <div className="flex gap-3 pt-2">
            <button type="button" onClick={onClose} className="flex-1 py-3 rounded-2xl border border-gray-200 text-gray-600 font-bold text-sm hover:bg-gray-50 transition-all">Cancelar</button>
